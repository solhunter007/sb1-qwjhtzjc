/*
  ...
  1. Tables
    - Ensures profiles, creations, and votes tables exist with correct structure
    - Adds missing columns and constraints
  2. Storage
    - Ensures creations bucket exists with proper configuration
    - Updates storage policies with unique names
  3. Policies
    - Creates unique policies for all tables
    - Sets up proper RLS for all operations
*/
...
CREATE POLICY "profiles_read_final" ON profiles
  FOR SELECT TO public USING (true);

CREATE POLICY "profiles_insert_final" ON profiles
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "profiles_update_final" ON profiles
  FOR UPDATE TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "creations_read_final" ON creations
  FOR SELECT TO public USING (true);

CREATE POLICY "creations_insert_final" ON creations
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "creations_update_final" ON creations
  FOR UPDATE TO authenticated USING (auth.uid() = user_id);

CREATE POLICY "votes_read_final" ON votes
  FOR SELECT TO public USING (true);

CREATE POLICY "votes_insert_final" ON votes
  FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

CREATE POLICY "votes_delete_final" ON votes
...
CREATE POLICY "creations_storage_read_final"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'creations');

CREATE POLICY "creations_storage_insert_final"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (bucket_id = 'creations');

CREATE POLICY "creations_storage_update_final"
ON storage.objects FOR UPDATE
TO authenticated
USING (bucket_id = 'creations')
WITH CHECK (bucket_id = 'creations');

CREATE POLICY "creations_storage_delete_final"
